...
if not defined MSYS2_NOSTART (
  start "%CONTITLE%" "%WD%mintty" -i /appurus.ico -t "%CONTITLE%" "/usr/bin/%LOGINSHELL%" --login %SHELL_ARGS%
) else (
  "%WD%mintty" -i /appurus.ico -t "%CONTITLE%" "/usr/bin/%LOGINSHELL%" --login %SHELL_ARGS%
)
exit /b %ERRORLEVEL%
...
if not defined MSYS2_NOSTART (
  start "%CONTITLE%" "%ComEmuCommand%" /Here /Icon "%WD%..\..\appurus.ico" /cmd "%WD%\%LOGINSHELL%" --login %SHELL_ARGS%
) else (
  "%ComEmuCommand%" /Here /Icon "%WD%..\..\appurus.ico" /cmd "%WD%\%LOGINSHELL%" --login %SHELL_ARGS%
)
exit /b %ERRORLEVEL%
