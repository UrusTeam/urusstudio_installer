...
busybox sh -c '
    ...
    if [ -e ../archives/msys.db ] ; then
        ...
        wget -P ../archives http://repo.msys2.org/distrib/i686/msys2-base-i686-20161025.tar.xz
        printf "$REPOURL ok\n"
        exit 0
    fi
    ...
    if [ -e ../archives/msys.db ] ; then
        ...
        rm -f ../archives/msys.db
        export REPOURL=$(cat ./baserepo)
        wget -P ../archives http://www2.futureware.at/~nickoe/msys2-mirror/distrib/i686/msys2-base-i686-20161025.tar.xz
        printf "$REPOURL ok\n"
        exit 0
    fi'
